...
def shared_model(vec, weights, args):
    ...
    ac_embs = layers.Embedding(ac_weights.shape[0],
                               ac_weights.shape[1],
                               weights=[ac_weights],
                               #the one hot encoded activity weight matrix used as the initial weight matrix
    ...
    if os.getenv("PLOT_MODEL", False) in (True, "T", "True", "true", "t", 1, "1"):
        plot_model(
            shared,
...
def shared_model_fit(vec_train, shared, MY_WORKSPACE_DIR, batch_size, epochs, args):
    ...
    model_inputs, model_outputs = generate_inputs_shared(vec_train, args)
...
def generate_inputs_shared(vec, args):
    prefix_len = args['prefix_length']
